...
#fitting the model 
results_C = model.fit(train_ds,
epochs=5,
batch_size=100,
validation_data=val_ds)


df = pd.DataFrame(results_C.history)
df.to_csv('results_C_model.csv')

#saving model for later use
model.save('emotions_v2.h5')
...
plt.plot(results_C.history['loss'], label="traning loss")
plt.plot(results_C.history['val_loss'], label="validation loss")
plt.show()
plt.savefig('learning_curve_C')