...
#fitting the model 
result_BL=model.fit(train_ds,
epochs=25,
batch_size=100,
validation_data=val_ds)
df = pd.DataFrame(result_BL.history)
df.to_csv('result_BL_model.csv')
...
for i in range(9):
  ...
  plt.axis("off")
plt.show()
plt.savefig('prediction_validation_cure_BL')
...
for i in range(9):
  ...
  plt.axis("off")
plt.show()
plt.savefig('prediction_test_cure_BL')

plt.plot(result_BL.history['loss'], label="traning loss")
plt.plot(result_BL.history['val_loss'], label="validation loss")
plt.show()
plt.savefig('learning_cure_BL')